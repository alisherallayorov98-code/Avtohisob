...
router.post('/vehicles/:id/intervals', authorize('admin', 'super_admin', 'manager'), createInterval);

// Odometer quick-update: PATCH /api/service-intervals/vehicles/:id/odometer
router.patch('/vehicles/:id/odometer', authorize('admin', 'super_admin', 'manager'), updateVehicleOdometer);

// Per-interval: PATCH/POST/DELETE /api/service-intervals/:id
router.patch('/:id', authorize('admin', 'super_admin', 'manager'), updateInterval);
router.post('/:id/complete', authorize('admin', 'super_admin', 'manager'), completeService);
