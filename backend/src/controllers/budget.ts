...
export async function upsertBudget(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const { year, month, category, amount, branchId } = req.body
    if (!year || month === undefined || !category || amount === undefined) {
      throw new AppError('year, month, category, amount majburiy', 400)
    }
...
export async function getBudgetActual(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const year = parseInt(req.query.year as string) || new Date().getFullYear()
